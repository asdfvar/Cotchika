...
def generate_cost (domain, resistance, start, destination):
   ...
   while destination not in perimeter and len (perimeter) > 0:
      ...
      for cell in perimeter:
         dest_dist = la.norm (np.array (destination) - np.array (cell)) + resistance[cell]
      ...
      for dir_count in range (3**domain.ndim):
         ...
         all (np.array (new_cell) < np.shape (domain)) and \
         domain[new_cell]:
            ...
            new_cost = cell_cost + step_dist + resistance[new_cell]
